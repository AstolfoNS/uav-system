...
- 本地联调示例:
  ...
  - `http://127.0.0.1:8111/api/v1/swagger-ui/index.html`
...
- Header（建议）:
  - `Authorization: Bearer <access_token>`
  - `Refresh-Token: Bearer <refresh_token>`
- 说明：未携带 `Authorization` 也可访问该路径，但建议携带登录态以完成服务端令牌清理。
- 返回: `R<Void>`
...
2. 使用 accessToken 查询个人信息

```bash
curl -X GET "http://localhost:8111/api/v1/users/profile" \
...
```bash
curl -X GET "http://localhost:8111/api/v1/yolo-nodes/page?current=1&size=10" \
  -H "Authorization: Bearer <access_token>"
```
...
```bash
curl -X GET "http://localhost:8111/api/v1/inference/records/page?current=1&size=10" \
  -H "Authorization: Bearer <access_token>"
```
