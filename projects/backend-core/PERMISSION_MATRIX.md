...
- 本文表格中的路径均为 controller 相对路径，未重复书写全局前缀。
- 实际完整访问路径为：`/api/v1` + 表格路径。
  - 例如：`/users/profile` 的完整路径为 `/api/v1/users/profile`。
...
| 认证 | POST | /auth/logout  | 无（路径放行；建议携带 Authorization） |
